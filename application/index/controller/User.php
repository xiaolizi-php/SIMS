...
class User extends Frontend
{
    ...
    {
        $examinee = $this->request->post("examinee");
        $nickname = $this->request->post("nickname");
        $user = $this->auth->getUser();
        $avatar = $this->request->request('avatar', '', 'trim,strip_tags,htmlspecialchars');
        if ($examinee) {
            $exists = \app\common\model\User::where('examinee', $examinee)->where('id', '<>', $this->auth->id)->find();
            if ($exists) {
                $this->error(__('examinee already exists'));
            }
    ...
    {
        $examinee =$this->auth->examinee;
        if($examinee==null){
            $this->view->assign('title', __('准考证绑定'));
            return $this->view->fetch('examinee');
        }
        else{
    ...
    {
        $examinee  =$this->auth->examinee; 
        $list= db('result')->where('number','=',$examinee)->select();
        if($list==null){
            $this->error(__('暂时无法查询成绩'));
        }else
        $this->view->assign('list',$list);
        $this->view->assign('title', __('Personal achievement'));
        return $this->view->fetch();
    }
    /**
     * 修改密码
